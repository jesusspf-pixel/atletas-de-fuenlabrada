...
export const onRequestPost:PagesFunction<Env>=async({request,env})=>{
  ...
  if(athleteIds.length){
    ...
    feedback=await rows<any>(`${env.SUPABASE_URL}/rest/v1/athlete_training_feedback?athlete_id=in.${encodeURIComponent(encoded)}&session_date=gte.${since.toISOString().slice(0,10)}&select=athlete_id,session_date,duration_minutes,distance_m,rpe,fatigue_feeling,pain_or_discomfort,sleep_quality`,{headers:serviceHeaders});
  }
  const aggregate={athletes:athleteIds.length,runnersWithData:new Set(feedback.map(x=>x.athlete_id)).size,manualSessions:feedback.length,totalKm:Math.round(feedback.reduce((sum,x)=>sum+Number(x.distance_m||0),0)/100)/10,totalMinutes:Math.round(feedback.reduce((sum,x)=>sum+Number(x.duration_minutes||0),0)),averageRpe:feedback.length?Math.round(feedback.reduce((sum,x)=>sum+Number(x.rpe||0),0)/feedback.length*10)/10:null,feedbackEntries:feedback.length,painReports:feedback.filter(x=>x.pain_or_discomfort).length,averageFatigue:feedback.length?Math.round(feedback.reduce((sum,x)=>sum+Number(x.fatigue_feeling||0),0)/feedback.length*10)/10:null};
  const content:any[]=[{type:"text",text:`Genera el borrador de la semana ${input.weekStartsOn} para ${group.name}. Horario habitual: ${group.schedule_days||"no indicado"}, ${group.starts_at||""}-${group.ends_at||""}. Punto de partida: ${clean(input.startingPoint)}. Objetivo: ${clean(input.objective)}. Fecha objetivo: ${clean(input.targetDate,40)||"no indicada"}. Condicionantes: ${clean(input.constraints)||"ninguno indicado"}. Planes anteriores o criterio pegado por el entrenador: ${clean(input.previousPlans,12000)||"no aportado"}. Datos agregados y no identificativos introducidos manualmente en la plataforma durante las últimas 8 semanas (no contienen datos de Strava): ${JSON.stringify(aggregate)}.`}];
  ...
  const system=`Eres copiloto de un entrenador de atletismo experto. Tu trabajo es proponer, nunca publicar ni sustituir su criterio. Usa los planes aportados para respetar su metodología. Ajusta progresión, recuperación y especificidad al objetivo; si la cobertura de datos es baja, sé conservador. No identifiques atletas ni hagas diagnósticos. Devuelve exclusivamente JSON válido con {"title":"...","sessions":[session],"rationale":"..."}. Incluye exactamente una session por cada día de Lunes a Domingo. Cada session debe incluir strings: day,objective,warmup,technique,main,cooldown,notes,coachNotes,duration,rpe,volume,intensity. intensity solo puede ser Baja, Media, Alta o Competición. Duración y RPE deben ser strings numéricos. Los campos objective,warmup,technique,main,cooldown y notes son el plan que verá el atleta: redacta instrucciones breves, directas, profesionales y fáciles de ejecutar. En technique indica solo el tipo de trabajo y su duración, sin enumerar ejercicios. En main usa expresiones naturales como «a un ritmo que permita conversar» y zonas de intensidad cuando proceda; elimina explicaciones sobre organización interna o niveles. En cooldown indica solo trote, movilidad o estiramientos y duración. notes debe contener como máximo dos frases útiles para el grupo. coachNotes es privado y solo lo ve el entrenador: coloca ahí ejercicios técnicos concretos, organización por niveles, correcciones, material, vigilancia de molestias y recordatorios metodológicos. Deja vacíos los campos públicos de los días de descanso, excepto notes si conviene explicar el descanso.`;
  const sessionSchema={type:"object",additionalProperties:false,properties:{objective:{type:"string"},warmup:{type:"string"},technique:{type:"string"},main:{type:"string"},cooldown:{type:"string"},notes:{type:"string"},coachNotes:{type:"string"},duration:{type:"string"},rpe:{type:"string"},volume:{type:"string"},intensity:{type:"string",enum:["Baja","Media","Alta","Competición"]}},required:["objective","warmup","technique","main","cooldown","notes","coachNotes","duration","rpe","volume","intensity"]};
  const outputSchema={type:"object",additionalProperties:false,properties:{title:{type:"string"},sessions:{type:"array",items:{...sessionSchema,properties:{day:{type:"string",enum:["Lunes","Martes","Miércoles","Jueves","Viernes","Sábado","Domingo"]},...sessionSchema.properties},required:["day",...sessionSchema.required]}},rationale:{type:"string"}},required:["title","sessions","rationale"]};
  const callClaude=async(model:string)=>{
    ...
    const response=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:anthropicHeaders,body:JSON.stringify({model,max_tokens:6500,temperature:0,system,messages:[{role:"user",content}],output_config:{format:{type:"json_schema",schema:outputSchema}}})});
  ...
  try{const parsed=JSON.parse(raw);if(!Array.isArray(parsed?.sessions))throw new Error("invalid");const sessions=Object.fromEntries(parsed.sessions.map((session:any)=>[clean(session.day,20),session]));if(!sessions.Lunes||!sessions.Domingo)throw new Error("invalid");return json({title:clean(parsed.title,140)||`Running A · Semana ${input.weekStartsOn}`,sessions,rationale:clean(parsed.rationale,1200)});}catch{return json({error:"La propuesta no llegó con el formato esperado. Vuelve a intentarlo."},502)}
};
