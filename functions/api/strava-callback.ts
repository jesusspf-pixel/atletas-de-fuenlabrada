...
const redirect = (params: Record<string, string>) => Response.redirect(`${CANONICAL_ORIGIN}/deportivo?${new URLSearchParams(params).toString()}`, 302);
...
export async function onRequestGet(context: any) {
  ...
  if (!env.STRAVA_CLIENT_ID || !env.STRAVA_CLIENT_SECRET || !env.SUPABASE_URL || !env.SUPABASE_SERVICE_ROLE_KEY) return redirect({ strava: "config_error" });
  const code = requestUrl.searchParams.get("code");
  const state = requestUrl.searchParams.get("state");
  const error = requestUrl.searchParams.get("error");
  if (error || !code || !state) return redirect({ strava: "cancelled" });
  ...
  if (!savedState?.athlete_id || !savedState.profile_id || !savedState.expires_at || new Date(savedState.expires_at).getTime() < Date.now()) return redirect({ strava: "invalid_state" });
  ...
  const token = await tokenResponse.json().catch(() => null) as { access_token?: string; refresh_token?: string; expires_at?: number; scope?: string; athlete?: { id?: number; firstname?: string; lastname?: string; profile?: string; profile_medium?: string } } | null;
  if (!tokenResponse.ok || !token?.access_token || !token.refresh_token || !token.expires_at || !token.athlete?.id) return redirect({ strava: "token_error" });

  const displayName = [token.athlete.firstname, token.athlete.lastname].filter(Boolean).join(" ").trim() || `Strava ${token.athlete.id}`;
  const integrationResponse = await fetch(`${env.SUPABASE_URL}/rest/v1/athlete_external_integrations?on_conflict=athlete_id,provider`, {
    ...
    body: JSON.stringify({ athlete_id: savedState.athlete_id, provider: "strava", provider_athlete_id: String(token.athlete.id), provider_display_name: displayName, provider_avatar_url: token.athlete.profile || token.athlete.profile_medium || null, scopes: (token.scope || "activity:read").split(/[ ,]+/).filter(Boolean), status: "connected", connected_by: savedState.profile_id, connected_at: new Date().toISOString(), updated_at: new Date().toISOString() }),
  });
  const [integration] = await integrationResponse.json().catch(() => []) as { id?: string }[];
  if (!integrationResponse.ok || !integration?.id) return redirect({ strava: "save_error" });
  ...
  if (!tokenSave.ok) return redirect({ strava: "save_error" });
  return redirect({ athleteId: savedState.athlete_id, strava: "connected" });
}
