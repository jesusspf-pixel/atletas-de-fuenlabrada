...
export const onRequestPost:PagesFunction<Env>=async({request,env})=>{
  ...
  if(latestCached&&(latestCached.input_signature===inputSignature||latestCached.analysis_date===today))return json({insight:latestCached.insight,cached:true,analysisDate:latestCached.analysis_date});
  ...
  if(monthlyUsage>=monthlyLimit)return latestCached?json({insight:latestCached.insight,cached:true,limitReached:true,analysisDate:latestCached.analysis_date}):json({error:"Se ha alcanzado el límite mensual de análisis del club."},429);
