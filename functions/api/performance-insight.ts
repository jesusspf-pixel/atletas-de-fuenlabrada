...
export const onRequestPost:PagesFunction<Env>=async({request,env})=>{
  ...
  if(monthlyUsage>=monthlyLimit)return latestCached?json({insight:latestCached.insight,cached:true,analysisDate:latestCached.analysis_date}):json({error:"El análisis inteligente se actualizará más adelante."},429);
