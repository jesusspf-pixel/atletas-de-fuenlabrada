...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  for (const invitation of invitations) {
    ...
    const text = `Hola:\n\nTu acceso al Club Atletas de Fuenlabrada está preparado. Completa tu ficha familiar y la de los atletas a tu cargo desde este enlace personal:\n\n${link}\n\nLa matrícula ya consta como abonada y no volverá a cobrarse durante este proceso. El enlace caduca en 30 días.\n\nClub Atletas de Fuenlabrada`;
    const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { authorization: `Bearer ${env.RESEND_API_KEY}`, "content-type": "application/json" }, body: JSON.stringify({ from: "Club Atletas de Fuenlabrada <info@atletasdefuenlabrada.com>", reply_to: "info@atletasdefuenlabrada.com", to: [invitation.email], subject: "Completa tu renovación · Atletas de Fuenlabrada", text, html: `<div style="font-family:Arial,sans-serif;color:#10233f;line-height:1.6;max-width:620px"><h2 style="color:#1559b2">Tu acceso al club ya está preparado</h2><p>Hola:</p><p>Como miembro del Club Atletas de Fuenlabrada, puedes completar tu ficha familiar y la de los atletas a tu cargo mediante el siguiente enlace personal.</p><p><strong>La matrícula ya consta como abonada</strong>, por lo que no volverá a cobrarse durante este proceso.</p><p><a href="${safe(link)}" style="display:inline-block;padding:13px 20px;border-radius:10px;background:#1559b2;color:#fff;text-decoration:none;font-weight:bold">Completar mi renovación</a></p><p>Si el botón no funciona, copia y pega este enlace en el navegador:<br><a href="${safe(link)}">${safe(link)}</a></p><p>Este enlace es personal y caduca en 30 días.</p><p>Club Atletas de Fuenlabrada</p></div>` }) });
    if (response.ok) {
      sent += 1;
