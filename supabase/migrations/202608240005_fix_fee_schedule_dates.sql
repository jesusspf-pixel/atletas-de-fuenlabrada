-- Calendario definitivo: día 5, septiembre día 10 (o la fecha de alta tardía).
...
begin
  ...
  select coalesce(a.user_profile_id,f.primary_profile_id) into payer from public.athletes a left join public.families f on f.id=a.family_id where a.id=m.athlete_id;
  ...
  if m.plan='monthly' then
    ...
    elsif extract(month from current_date)=9 and extract(day from current_date)<=r.full_rate_through_day then first_due:=current_date;
    ...
    else first_due:=current_date;
    end if;
    due:=first_due;
    while due<=make_date(season_start+1,6,5) loop
      amount:=r.monthly_cents;
      if due=first_due and extract(day from current_date)>r.full_rate_through_day and extract(day from current_date)<=r.half_rate_through_day then amount:=round(r.monthly_cents/2.0); end if;
      ...
      values(m.id,m.athlete_id,payer,'recurring',date_trunc('month',due)::date,period_end,due,amount,amount,'awaiting_admin',jsonb_build_object('plan','monthly','automatic',true)) on conflict do nothing;
    ...
    foreach due in array array[case when current_date<=make_date(season_start,9,10) then make_date(season_start,9,10) when extract(month from current_date) between 9 and 11 then current_date else null end,make_date(season_start,12,5),make_date(season_start+1,3,5)] loop
      if due is not null and due>=current_date then
        ...
        insert into public.billing_charge_drafts(membership_id,athlete_id,payer_profile_id,charge_kind,period_starts_on,period_ends_on,scheduled_for,calculated_amount_cents,approved_amount_cents,status,calculation_snapshot)
        values(m.id,m.athlete_id,payer,'recurring',date_trunc('month',due)::date,period_end,due,amount,amount,'awaiting_admin',jsonb_build_object('plan','term','automatic',true)) on conflict do nothing;
      end if;
    end loop;
  end if;
end $$;
grant execute on function public.rebuild_membership_fee_schedule(uuid) to authenticated;
...
begin
 ...
 if waive_enrolment then insert into public.billing_charge_drafts(membership_id,athlete_id,payer_profile_id,charge_kind,scheduled_for,calculated_amount_cents,approved_amount_cents,status,calculation_snapshot) values(m.id,target_athlete_id,payer,'enrolment',current_date,0,0,'waived',jsonb_build_object('reason','Matrícula exenta')) on conflict do nothing;
 else insert into public.billing_charge_drafts(membership_id,athlete_id,payer_profile_id,charge_kind,scheduled_for,calculated_amount_cents,approved_amount_cents,status,calculation_snapshot) values(m.id,target_athlete_id,payer,'enrolment',current_date,r.enrolment_cents,r.enrolment_cents,'approved',jsonb_build_object('reason','Alta nueva')) returning id into enrolment_draft_id; end if;
