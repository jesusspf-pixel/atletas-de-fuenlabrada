-- Alta guiada: grupo de entrenamiento elegido por la familia y ficha FAM visible para el club.
create table if not exists public.federation_license_applications (
  id uuid primary key default gen_random_uuid(),
  athlete_id uuid not null unique references public.athletes(id) on delete cascade,
  status text not null default 'pending_review' check (status in ('pending_review','ready_to_file','filed','completed','not_required')),
...
alter table public.federation_license_applications enable row level security;
drop policy if exists "fam applications admin read" on public.federation_license_applications;
create policy "fam applications admin read" on public.federation_license_applications
  for select using (public.is_admin());
drop policy if exists "fam applications admin manage" on public.federation_license_applications;
create policy "fam applications admin manage" on public.federation_license_applications
  for all using (public.is_admin()) with check (public.is_admin());

create or replace function public.category_for_year(target_birth_date date, target_year integer)
returns text language plpgsql immutable as $$
declare target_age integer := target_year - extract(year from target_birth_date)::integer;
begin
  if target_age <= 5 then return 'Sub 6'; end if;
  if target_age <= 7 then return 'Sub 8'; end if;
  if target_age <= 9 then return 'Sub 10'; end if;
  if target_age <= 11 then return 'Sub 12'; end if;
  if target_age <= 13 then return 'Sub 14'; end if;
  if target_age <= 15 then return 'Sub 16'; end if;
  if target_age <= 17 then return 'Sub 18'; end if;
  if target_age <= 19 then return 'Sub 20'; end if;
  if target_age <= 22 then return 'Sub 23'; end if;
...
begin
  ...
  for v_athlete in select value from jsonb_array_elements(payload->'athletes') loop
    ...
    values (v_family_id,v_athlete->>'first_name',v_athlete->>'last_name',(v_athlete->>'birth_date')::date,v_athlete->>'federative_sex',nullif(v_athlete->>'dni_nie',''),'pending_review','pending',nullif(v_athlete->>'health_notes',''),v_group_id,v_training_category,v_competition_category) returning id into v_athlete_id;
  ...
  insert into public.audit_log(actor_id,entity_type,entity_id,action,metadata) values(v_profile_id,'family',v_family_id,'registration_submitted',jsonb_build_object('athletes',jsonb_array_length(payload->'athletes')));
  return v_family_id;
end $$;
...
declare
  ...
  v_training_category text; v_competition_category text;
begin
  ...
  on conflict(id) do update set full_name=excluded.full_name,phone=excluded.phone,updated_at=now();
  insert into public.athletes(user_profile_id,first_name,last_name,birth_date,federative_sex,dni_nie,club_status,license_status,medical_notes,training_group_id,training_category,official_competition_category)
  values(v_profile_id,payload->>'first_name',payload->>'last_name',(payload->>'birth_date')::date,payload->>'federative_sex',nullif(payload->>'dni_nie',''),'pending_review','pending',nullif(payload->>'health_notes',''),v_group_id,v_training_category,v_competition_category) returning id into v_athlete_id;
  insert into public.health_declarations(athlete_id,relevant_condition,relevant_condition_detail,asthma_allergy_medication,injury_limitation,support_needs,additional_notes,declared_by)
