...
const USER_AGENT = "Club Atletas de Fuenlabrada official-results collector/1.0";
...
function absolute(href) {
  return new URL(href.replace(/&amp;/g, "&"), FAM_ORIGIN).toString();
}
...
function calendarRows(html, year) {
  ...
  while ((match = rowRegex.exec(html))) {
    ...
    rows.push({
      ...
      calendar_url: "",
      results_url: absolute(resultLink[1]),
...
async function scanFamMonth(env, date) {
  ...
  const rows = calendarRows(html, year).map(row => ({ ...row, calendar_url: calendarUrl }));
  if (rows.length) {
...
async function runCollector(env) {
  ...
  const settings = await supabase(env, "federation_import_settings?id=eq.true&select=history_cursor_month,history_from");
  const setting = settings?.[0];
  if (!setting) throw new Error("No existe la configuración de importación federativa.");
  const now = new Date();
  const current = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1));
  const cursor = monthAt(setting.history_cursor_month || setting.history_from);
  const historical = sameOrBefore(cursor, current);
  const months = historical
    ? [cursor, addMonths(cursor, 1)].filter(month => sameOrBefore(month, current))
    : [current, addMonths(current, -1)];

  const summary = [];
  for (const month of months) {
    try {
      summary.push(await scanFamMonth(env, month));
    } catch (error) {
      summary.push({ month: month.toISOString().slice(0, 7), error: error instanceof Error ? error.message : "Error desconocido" });
    }
  }

  const nextCursor = historical ? addMonths(months[months.length - 1], 1).toISOString().slice(0, 10) : setting.history_cursor_month;
  ...
  await supabase(env, "federation_import_settings?id=eq.true", {
    ...
    body: JSON.stringify({
      history_cursor_month: nextCursor,
      fam_last_scan_at: new Date().toISOString(),
      import_job_last_error: errors || null
    })
  });
  console.log(JSON.stringify({ event: "fam_calendar_scan", historical, summary }));
  return { historical, summary, nextCursor };
}

export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname === "/health") return Response.json({ ok: true, service: "club-atletas-results-collector" });
    return Response.json({
      ok: true,
      message: "El colector se ejecuta de forma programada lunes y jueves a las 05:30 UTC."
    });
  },
  async scheduled(_controller, env, ctx) {
    ctx.waitUntil(runCollector(env).catch(error => console.error(JSON.stringify({
      event: "fam_calendar_scan_failed",
