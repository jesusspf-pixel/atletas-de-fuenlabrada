...
function Demo() {
  ...
  return <div className="demo-shell">
    ...
    <header><div><small>SPORTMED / CLUB EXPERIENCE</small><h1>Tu club, en movimiento</h1><p>La experiencia digital que conecta, organiza y hace crecer tu club.</p></div><button onClick={reset}>Reiniciar demo</button></header>
    ...
    <main>
      ...
      {tab === "Avisos" && <><h2>El club, al día</h2><article><h3>Bienvenidos a la nueva temporada</h3><p>Este mensaje forma parte de la demostración comercial de Sportmed.</p></article>{sent.map((s,i) => <article key={i}><small>PUBLICACIÓN SIMULADA</small><p>{s}</p></article>)}{staff && <form onSubmit={e => { e.preventDefault(); if (!message.trim()) return; setSent(s => [...s,message.trim()]); setMessage(""); setNotice("Aviso añadido a esta demo. No se han enviado correos ni notificaciones."); }}><label>Nuevo aviso de ejemplo<textarea required maxLength={1000} value={message} onChange={e => setMessage(e.target.value)}/></label><button>Publicar solo en la demo</button></form>}</>}
