...
function CoachSports({ profile, groups, athletes, avatars, selectedGroupId, setSelectedGroupId, selected, selectAthlete }: { profile: Profile; groups: Group[]; athletes: Athlete[]; avatars:Record<string,string>; selectedGroupId: string; setSelectedGroupId: (id: string) => void; selected: Athlete | null; selectAthlete: (id: string) => void }) {
  ...
  return <><div className="page-head"><div><h1>Mis grupos</h1><p>Selecciona un grupo para abrirlo en su propia pantalla.</p></div></div><section className="cards">{groups.map(group=>{const athleteCount=athletes.filter(athlete=>athlete.training_group_id===group.id).length;return <button key={group.id} className="panel group-card group-button" onClick={()=>setSelectedGroupId(group.id)}><i style={{background:group.colour}}/><h2>{group.name}</h2><p>{group.category_label}</p><b className="coach-group-count">{athleteCount} atleta{athleteCount===1?"":"s"}</b><small>{group.schedule_days||"Horario pendiente"}</small></button>})}</section></>;
}
