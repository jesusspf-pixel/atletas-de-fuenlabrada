...
export default function SportsCenter() {
  ...
  return <main className="club-shell sports-center-shell"><aside className="club-side"><a className="sports-home-mark" href="/" aria-label="Volver al inicio">AF</a><small className="side-role">{profile.role === "coach" ? "Entrenador" : profile.role === "parent" ? "Familia" : profile.role === "owner" ? "Propietario" : profile.role === "admin" ? "Administrador" : "Atleta"}</small><nav>{profile.role !== "coach" && <button className={mode === "ranking" ? "selected" : ""} onClick={() => setMode("ranking")}>Ranking del club</button>}<button className={mode === "athletes" ? "selected" : ""} onClick={() => setMode("athletes")}>{profile.role === "coach" ? "Mis grupos" : "Atletas y marcas"}</button>{challengeAthleteId && <button className={mode === "challenge" ? "selected" : ""} onClick={() => setMode("challenge")}>Club Challenge</button>}</nav><div className="side-user"><b>{profile.full_name || profile.email}</b><small>Resultados, marcas y seguimiento</small><a className="button-link outline" href="/club">← Web del club</a></div></aside><section className="club-content"><header className="topbar"><span>Club Atletas de Fuenlabrada</span></header>{mode === "ranking" && profile.role !== "coach" ? <HistoricalRanking /> : mode === "challenge" && challengeAthleteId ? <><div className="page-head"><div><h1>Club Challenge</h1><p>Tu reto semanal y la clasificación del club.</p></div><button className="outline" onClick={() => setMode("athletes")}>Volver a atletas y marcas</button></div><ClubChallenge athleteId={challengeAthleteId} /></> : profile.role === "coach" ? <CoachSports profile={profile} groups={groups} athletes={visibleAthletes} selectedGroupId={selectedGroupId} setSelectedGroupId={id => { setSelectedGroupId(id); setSelectedAthleteId(""); }} selected={selected} selectAthlete={setSelectedAthleteId} /> : <MemberSports profile={profile} athletes={athletes} selected={selected} selectAthlete={setSelectedAthleteId} />}</section></main>;
}
