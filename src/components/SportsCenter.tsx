...
export default function SportsCenter() {
  ...
  return <main className={`club-shell sports-center-shell visual-next-shell ${profile.role === "coach" ? "coach-next-shell" : ["owner","admin"].includes(profile.role) ? "admin-next-shell" : "member-next-shell"}`}><aside className="club-side"><a className="sports-home-mark" href="/" aria-label="Volver al inicio">AF</a><small className="side-role">{profile.role === "coach" ? "Entrenador" : profile.role === "parent" ? "Familia" : profile.role === "owner" ? "Propietario" : profile.role === "admin" ? "Administrador" : "Atleta"}</small><nav><button className={mode === "ranking" ? "selected" : ""} onClick={() => setMode("ranking")}><AppNavIcon name="Ranking del club"/><span>Ranking</span></button><button className={mode === "athletes" ? "selected" : ""} onClick={() => setMode("athletes")}><AppNavIcon name="Mis grupos"/><span>{profile.role === "coach" ? "Mis grupos" : "Atletas y marcas"}</span></button>{(profile.role === "coach" || challengeAthleteId) && <button className={mode === "challenge" ? "selected" : ""} onClick={() => setMode("challenge")}><AppNavIcon name="Club Challenge"/><span>Challenge</span></button>}</nav><div className="side-user"><b>{profile.full_name || profile.email}</b><small>Resultados, marcas y seguimiento</small><a className="button-link outline" href="/club">← Web del club</a></div></aside><section className="club-content"><header className="topbar"><span>Club Atletas de Fuenlabrada</span></header><div className="visual-page-body">{mode === "ranking" ? <HistoricalRanking /> : mode === "challenge" && (profile.role === "coach" || challengeAthleteId) ? <><div className="page-head"><div><h1>Club Challenge</h1><p>Tu reto semanal y la clasificación del club.</p></div><button className="outline" onClick={() => setMode("athletes")}>Volver a atletas y marcas</button></div><ClubChallenge athleteId={challengeAthleteId || undefined} /></> : profile.role === "coach" ? <CoachSports profile={profile} groups={groups} athletes={visibleAthletes} avatars={athleteAvatars} selectedGroupId={selectedGroupId} setSelectedGroupId={id => { setSelectedGroupId(id); setSelectedAthleteId(""); }} selected={selected} selectAthlete={setSelectedAthleteId} /> : <MemberSports profile={profile} athletes={athletes} avatars={athleteAvatars} selected={selected} selectAthlete={setSelectedAthleteId} />}</div></section></main>;
}
