...
export default function SportsCenter() {
  ...
  return <main className="club-shell sports-center-shell"><aside className="club-side"><a className="sports-home-mark" href="/" aria-label="Volver al inicio">AF</a><small className="side-role">{profile.role === "coach" ? "Entrenador" : profile.role === "parent" ? "Familia" : profile.role === "owner" ? "Propietario" : profile.role === "admin" ? "Administrador" : "Atleta"}</small><nav><button className={mode === "ranking" ? "selected" : ""} onClick={() => setMode("ranking")}>Ranking del club</button><button className={mode === "athletes" ? "selected" : ""} onClick={() => setMode("athletes")}>{profile.role === "coach" ? "Mis grupos" : "Atletas y marcas"}</button>{(profile.role === "coach" || challengeAthleteId) && <button className={mode === "challenge" ? "selected" : ""} onClick={() => setMode("challenge")}>Club Challenge</button>}</nav><div className="side-user"><b>{profile.full_name || profile.email}</b><small>Resultados, marcas y seguimiento</small><a className="button-link outline" href="/club">← Web del club</a></div></aside><section className="club-content"><header className="topbar"><span>Club Atletas de Fuenlabrada</span></header>{mode === "ranking" ? <HistoricalRanking /> : mode === "challenge" && (profile.role === "coach" || challengeAthleteId) ? <><div className="page-head"><div><h1>Club Challenge</h1><p>Tu reto semanal y la clasificación del club.</p></div><button className="outline" onClick={() => setMode("athletes")}>Volver a atletas y marcas</button></div><ClubChallenge athleteId={challengeAthleteId || undefined} /></> : profile.role === "coach" ? <CoachSports profile={profile} groups={groups} athletes={visibleAthletes} selectedGroupId={selectedGroupId} setSelectedGroupId={id => { setSelectedGroupId(id); setSelectedAthleteId(""); }} selected={selected} selectAthlete={setSelectedAthleteId} /> : <MemberSports profile={profile} athletes={athletes} selected={selected} selectAthlete={setSelectedAthleteId} />}</section></main>;
}
...
function CoachAthlete({ profile, athlete, back }: { profile: Profile; athlete: Athlete; back:()=>void }) {
  ...
  return <section className="athlete-detail athlete-detail-v2"><div className="athlete-detail-v2__hero"><button className="outline" onClick={back}>← Volver al grupo</button><div className="athlete-detail-v2__identity"><span className="athlete-detail-v2__avatar">{athlete.first_name[0]}{athlete.last_name[0]}</span><div><small>FICHA DEL ATLETA</small><h1>{athlete.first_name} {athlete.last_name}</h1><p>{athlete.training_groups?.name || "Grupo pendiente"}</p></div></div><div className="athlete-detail-v2__facts"><span><small>Grupo</small><b>{athlete.training_groups?.name||"Sin asignar"}</b></span><span><small>Licencia</small><b>{displayLicense(athlete)}</b></span><span><small>Estado</small><b>En seguimiento</b></span></div></div><div className="athlete-detail-v2__primary"><AthleteMessage profile={profile} athlete={athlete}/><form className="panel stacked-form" onSubmit={saveNote}><p className="coach-v2__eyebrow">Seguimiento interno</p><h2>Nota privada</h2><p>Solo la ve el cuerpo técnico autorizado.</p><textarea required value={note} onChange={e=>setNote(e.target.value)} placeholder="Añade una observación para el equipo técnico…"/><button>Guardar nota</button>{notice&&<p className={notice.startsWith("Nota")?"success-note":"error-note"}>{notice}</p>}<div className="coach-note-list">{notes.map(item=><article key={item.id}><p>{item.body}</p><small>{new Date(item.created_at).toLocaleString("es-ES")}</small></article>)}</div></form></div><div className="athlete-detail-v2__section"><AthleteParticipation athleteId={athlete.id}/></div><div className="athlete-detail-v2__section"><AthleteResults athleteId={athlete.id} athleteName={`${athlete.first_name} ${athlete.last_name}`} canAddTraining coachProfileId={profile.id}/></div></section>;
}
