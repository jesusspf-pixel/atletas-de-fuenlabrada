...
export default function SportsCenter() {
  ...
  const mainMenu=profile.role==="coach"?["Inicio","Mi perfil","Mis grupos","Planificación","Asistencia","Carreras","Avisos","Club Challenge"]:["owner","admin"].includes(profile.role)?["Inicio","Atletas","Grupos","Cuotas","Carreras","Asistencia","Avisos","Invitaciones","Tienda","Configuración"]:["Inicio",profile.role==="parent"?"Mis atletas":"Mi perfil","Carreras","Cuotas","Avisos","Tienda","Challenge"];
...
function CoachSports({ profile, groups, athletes, avatars, selectedGroupId, setSelectedGroupId, selected, selectAthlete }: { profile: Profile; groups: Group[]; athletes: Athlete[]; avatars:Record<string,string>; selectedGroupId: string; setSelectedGroupId: (id: string) => void; selected: Athlete | null; selectAthlete: (id: string) => void }) {
  ...
  if(selected)return <CoachAthlete profile={profile} athlete={selected} avatar={avatars[selected.id]} back={()=>selectAthlete("")}/>;
...
function CoachAthlete({ profile, athlete, avatar, back }: { profile: Profile; athlete: Athlete; avatar?: string; back:()=>void }) {
  ...
  return <section className="athlete-detail"><div className="page-head"><div><button className="outline" onClick={back}>← Volver al grupo</button><h1>{athlete.first_name} {athlete.last_name}</h1><p>{athlete.training_groups?.name || "Grupo"} · Licencia {displayLicense(athlete)}</p></div></div><article className="panel coach-athlete-identity">{avatar?<img src={avatar} alt={`Foto de ${athlete.first_name} ${athlete.last_name}`}/>:<i>{athlete.first_name[0]}{athlete.last_name[0]}</i>}<div><small>DATOS DEL ATLETA</small><h2>{athlete.first_name} {athlete.last_name}</h2><p>Grupo: {athlete.training_groups?.name||"Sin asignar"} · Licencia: {displayLicense(athlete)}</p></div></article><div className="detail-grid"><AthleteMessage profile={profile} athlete={athlete}/><form className="panel stacked-form" onSubmit={saveNote}><h2>Nota privada</h2><p>Solo la ve el cuerpo técnico autorizado.</p><textarea required value={note} onChange={e=>setNote(e.target.value)}/><button>Guardar nota</button>{notice&&<p className={notice.startsWith("Nota")?"success-note":"error-note"}>{notice}</p>}<div className="coach-note-list">{notes.map(item=><article key={item.id}><p>{item.body}</p><small>{new Date(item.created_at).toLocaleString("es-ES")}</small></article>)}</div></form></div><CoachTrainingFeedback athleteId={athlete.id}/><AthleteParticipation athleteId={athlete.id}/><AthleteResults athleteId={athlete.id} athleteName={`${athlete.first_name} ${athlete.last_name}`} canAddTraining coachProfileId={profile.id} canPlanFitness canRecordFitness={false}/></section>;
}
