...
function CoachAthlete({ profile, athlete, back }: { profile: Profile; athlete: Athlete; back:()=>void }) {
  ...
  return <section className="athlete-detail"><div className="page-head"><div><button className="outline" onClick={back}>← Volver al grupo</button><h1>{athlete.first_name} {athlete.last_name}</h1><p>{athlete.training_groups?.name || "Grupo"} · Licencia {displayLicense(athlete)}</p></div></div><article className="panel"><small>DATOS DEL ATLETA</small><h2>{athlete.first_name} {athlete.last_name}</h2><p>Grupo: {athlete.training_groups?.name||"Sin asignar"} · Licencia: {displayLicense(athlete)}</p></article><div className="detail-grid"><AthleteMessage profile={profile} athlete={athlete}/><form className="panel stacked-form" onSubmit={saveNote}><h2>Nota privada</h2><p>Solo la ve el cuerpo técnico autorizado.</p><textarea required value={note} onChange={e=>setNote(e.target.value)}/><button>Guardar nota</button>{notice&&<p className={notice.startsWith("Nota")?"success-note":"error-note"}>{notice}</p>}<div className="coach-note-list">{notes.map(item=><article key={item.id}><p>{item.body}</p><small>{new Date(item.created_at).toLocaleString("es-ES")}</small></article>)}</div></form></div><AthleteParticipation athleteId={athlete.id}/><AthleteResults athleteId={athlete.id} athleteName={`${athlete.first_name} ${athlete.last_name}`} canAddTraining coachProfileId={profile.id}/></section>;
}
