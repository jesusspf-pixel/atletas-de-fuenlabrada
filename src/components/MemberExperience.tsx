...
type GroupMate = { id: string; first_name: string; last_name: string; athlete_profile_settings?: { avatar_url: string | null }[] | null };
type GroupCoach = { coach_profile_id: string; profiles?: { full_name: string | null } | null };
...
export default function MemberExperience({ profileId }: { profileId: string }) {
  ...
  const openGroup = async () => {
    ...
    setShowGroup(true);
    const [mates, coachLinks] = await Promise.all([
      supabase.from("athletes").select("id,first_name,last_name,athlete_profile_settings(avatar_url)").eq("training_group_id", athlete.training_group_id).eq("club_status", "active").order("first_name"),
      supabase.from("training_group_coaches").select("coach_profile_id").eq("training_group_id", athlete.training_group_id),
    ]);
    setGroupMates((mates.data ?? []) as unknown as GroupMate[]);
    const coachIds = (coachLinks.data ?? []).map(link => link.coach_profile_id);
    if (!coachIds.length) return setGroupCoaches([]);
    const { data: coachProfiles } = await supabase.from("profiles").select("id,full_name").in("id", coachIds);
    setGroupCoaches(coachIds.map(id => ({ coach_profile_id: id, profiles: { full_name: coachProfiles?.find(item => item.id === id)?.full_name || null } })));
  };
  ...
  if (mode === "home") return <section className="design-v2-stage member-live-home"><header className="design-v2-hero"><div><small>INICIO · MI TEMPORADA</small><h1>Todo empieza<br/>aquí.</h1><p>{athlete.training_groups?.name || "Grupo pendiente"} · Licencia {licenseText(athlete)}</p></div></header><section className="design-v2-float">
    ...
    {showGroup && <div className="member-group-overlay" role="dialog" aria-modal="true"><section><header><div><small>TU EQUIPO</small><h2>{athlete.training_groups?.name}</h2><p>Entrenadores y atletas que forman tu grupo.</p></div><button aria-label="Cerrar" onClick={() => setShowGroup(false)}>×</button></header><div className="member-group-content"><div><h3>Entrenadores</h3><div className="member-roster">{groupCoaches.map(coach => <article key={coach.coach_profile_id}><span className="roster-avatar">{(coach.profiles?.full_name || "E").split(" ").map(word => word[0]).slice(0,2).join("")}</span><b>{coach.profiles?.full_name || "Entrenador pendiente de identificar"}</b><small>Entrenador</small></article>)}{!groupCoaches.length && <p>El entrenador todavía no está visible.</p>}</div></div><div><h3>Atletas</h3><div className="member-roster">{groupMates.map(mate => { const avatar = mate.id === athlete.id ? profileSettings?.avatar_url : mate.athlete_profile_settings?.[0]?.avatar_url; return <article key={mate.id}>{avatar ? <img src={avatar} alt="" /> : <span className="roster-avatar">{mate.first_name[0]}{mate.last_name[0]}</span>}<b>{mate.first_name} {mate.last_name}</b><small>{mate.id === athlete.id ? "Tú" : athlete.training_groups?.name}</small></article>})}{!groupMates.length && <p>No hay otros atletas visibles todavía.</p>}</div></div></div></section></div>}
  </section></section>;
