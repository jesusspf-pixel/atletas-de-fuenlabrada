...
export default function AthleteProfileEditor({ athleteId, canEdit }: { athleteId: string; canEdit: boolean }) {
  ...
  const load = async () => {
    ...
    if (data) {
      ...
      setSettings(next);
      setEditing(!(next.avatar_url || next.cover_url));
      notifyHeader(next);
    } else {
      setEditing(true);
    }
  ...
  const save = async (patch: Partial<Settings>) => {
    ...
    if (next.avatar_url || next.cover_url) setEditing(false);
  };
  ...
  const upload = async (event: ChangeEvent<HTMLInputElement>, kind: "avatar" | "cover") => {
    ...
    setBusy(true); setNotice(kind === "avatar" ? "Subiendo foto de perfil…" : "Subiendo imagen de portada…");
    ...
    event.target.value = "";
    setNotice(kind === "avatar" ? "Foto de perfil guardada y vinculada a tu ficha." : "Imagen de portada guardada y vinculada a tu ficha.");
  };
  ...
  return <article className="panel athlete-profile-card">
    {showLocalPreview && <div className="athlete-cover" style={settings.cover_url ? { backgroundImage: `url(${settings.cover_url})` } : undefined}>
    ...
    {canEdit && !editing && <div className="profile-editor-collapsed"><button type="button" className="outline" onClick={() => setEditing(true)}>Editar fotos y perfil</button></div>}
    {canEdit && editing && <div className="stacked-form"><div className="table-title"><h2>Personaliza tu ficha</h2>{(settings.avatar_url || settings.cover_url) && <button type="button" className="plain" onClick={() => setEditing(false)}>Cerrar edición</button>}</div><label>Foto de perfil<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" disabled={busy} onChange={event => void upload(event, "avatar")} /></label><label>Imagen de portada<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" disabled={busy} onChange={event => void upload(event, "cover")} /></label>{notice && <p className={success ? "success-note" : "error-note"}>{notice}</p>}<label>Presentación<textarea maxLength={280} value={settings.bio || ""} onChange={event => setSettings(current => ({ ...current, bio: event.target.value }))} placeholder="Cuéntanos algo sobre ti como atleta…" /></label><label className="check-line"><input type="checkbox" checked={settings.challenge_opt_in} onChange={event => setSettings(current => ({ ...current, challenge_opt_in: event.target.checked, show_activity_to_club: event.target.checked ? current.show_activity_to_club : false }))} />Participar en Club Challenge</label><label className="check-line"><input type="checkbox" disabled={!settings.challenge_opt_in} checked={settings.show_activity_to_club} onChange={event => setSettings(current => ({ ...current, show_activity_to_club: event.target.checked }))} />Compartir mis estadísticas de actividad con el club</label><small>Las fotos se guardan al seleccionarlas. Se admiten imágenes de hasta 12 MB. Usa Guardar cambios para la presentación y las preferencias del Challenge.</small><button type="button" disabled={busy} onClick={() => void save(settings)}>{busy ? "Guardando…" : "Guardar cambios"}</button></div>}
