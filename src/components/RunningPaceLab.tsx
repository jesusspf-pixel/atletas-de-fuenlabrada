...
export default function RunningPaceLab({activities}:{activities:Run[]}){
 ...
 const model=useMemo(()=>{
  ...
  return {samples:samples.length,weeklyKm,longest,five:reference,ten,half,marathon,tenPace,confidence};
 },[activities]);
 ...
 return <article className="pace-lab">
  ...
  </>:<div className="goal-calculator"><label>Prueba objetivo<select value={goalDistance} onChange={e=>setGoalDistance(e.target.value)}><option value="5">5 kilómetros</option><option value="10">10 kilómetros</option><option value="21.0975">Media maratón · 21,1 km</option><option value="42.195">Maratón · 42,2 km</option></select></label><label>Tiempo objetivo (h:mm:ss)<input value={goalTime} onChange={e=>setGoalTime(e.target.value)}/></label><article><small>Ritmo de competición</small><b>{goalPace?pace(goalPace):"—"}</b></article></div>}
