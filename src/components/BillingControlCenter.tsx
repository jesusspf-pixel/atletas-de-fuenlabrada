  ...
  return <section className="billing-control">
    ...
    <section className="two-columns">
      <form className="panel stacked-form" onSubmit={saveRules}>
        ...
        <label>Marzo–junio (€)<input type="number" min="0" step="0.01" value={field(rules.term_spring_cents)} onChange={e => setRules({ ...rules, term_spring_cents: Math.round(Number(e.target.value || 0) * 100) })} /></label>
        <label>Día completo hasta<input type="number" min="1" max="28" value={rules.full_rate_through_day} onChange={e => setRules({ ...rules, full_rate_through_day: Number(e.target.value) })} /></label>
        <label>Media cuota hasta<input type="number" min="1" max="28" value={rules.half_rate_through_day} onChange={e => setRules({ ...rules, half_rate_through_day: Number(e.target.value) })} /></label>
        <label>Descuento mensual desde atletas activos<input type="number" min="1" value={rules.family_discount_members} onChange={e => setRules({ ...rules, family_discount_members: Number(e.target.value) })} /></label>
