...
export default function BillingControlCenter() {
  ...
  return <section className="billing-control">
    ...
    <section className="panel table"><h2>Control financiero de cuotas</h2>{drafts.map(draft => <div className={`row billing-draft-row status-${draft.status}`} key={draft.id}><span><button className="plain" onClick={() => setSelectedMembershipId(draft.membership_id)}><b>{draft.athletes?.first_name} {draft.athletes?.last_name}</b><small>Ver sus cuotas →</small></button><small>{draft.charge_kind === "enrolment" ? "Matrícula" : "Cuota"} · {draft.memberships?.plan === "monthly" ? "Mensual" : "Trimestral"} · prevista {new Date(draft.scheduled_for).toLocaleDateString("es-ES")}</small></span><span><small>Calculado</small><b>{euro(draft.calculated_amount_cents)}</b>{draft.discount_cents > 0 && <small>Descuento: {euro(draft.discount_cents)}</small>}</span><span><small>Final</small><b>{euro(draft.approved_amount_cents ?? draft.calculated_amount_cents)}</b><small>{draft.status}</small></span><span>{actions(draft)}</span></div>)}{!drafts.length && <p>Aún no hay cuotas preparadas. Crea el primer borrador arriba.</p>}</section>{selectedMembershipId && <section className="panel"><h2>Cuotas del atleta</h2><button className="outline" onClick={() => setSelectedMembershipId("")}>Cerrar</button>{selectedDrafts.map(draft => <div className={`row billing-draft-row status-${draft.status}`} key={draft.id}><span><b>{draft.athletes?.first_name} {draft.athletes?.last_name}</b><small>{draft.charge_kind === "enrolment" ? "Matrícula" : "Cuota"} · {new Date(draft.scheduled_for).toLocaleDateString("es-ES")} · {draft.status}</small></span><span><b>{euro(draft.approved_amount_cents ?? draft.calculated_amount_cents)}</b><small>{draft.override_reason || "Sin ajuste"}</small></span><span>{actions(draft)}</span></div>)}</section>}</>}
  </section>;
}
