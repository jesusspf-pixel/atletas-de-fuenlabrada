...
type Integration = { id: string; athlete_id: string; provider: string; provider_athlete_id: string | null; status: string; connected_at: string; last_synced_at: string | null };
...
export default function ExternalSports({ athleteId }: { athleteId: string }) {
  ...
  const load = async () => {
    ...
    const [{ data: athlete }, { data: integrations }, { data: activityData }] = await Promise.all([
      ...
      client.from("athlete_external_integrations").select("id,athlete_id,provider,provider_athlete_id,status,connected_at,last_synced_at").eq("athlete_id", athleteId).eq("provider", "strava").maybeSingle(),
  ...
  return <section className="external-sports">
    ...
    <ClubChallenge athleteId={athleteId} />
  </section>;
}
