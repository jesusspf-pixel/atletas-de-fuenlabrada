    ...
    const enhance = () => {
      ...
      if (role === "coach") {
        document.querySelectorAll<HTMLElement>(".club-content .group-card").forEach(card => {
          if (card.dataset.sportsLinked === "true") return; card.dataset.sportsLinked = "true"; card.style.cursor = "pointer"; card.setAttribute("role", "button"); card.setAttribute("tabindex", "0");
          const open = () => { const groupName = card.querySelector("h2")?.textContent?.trim() || ""; window.location.assign(`/deportivo${groupName ? `?groupName=${encodeURIComponent(groupName)}` : ""}`); };
      ...
      if (role === "parent") {
        document.querySelectorAll<HTMLElement>(".club-content .athlete-summary").forEach(card => {
          if (card.dataset.sportsLinked === "true") return; card.dataset.sportsLinked = "true";
          card.addEventListener("dblclick", () => { const name = card.querySelector("h2")?.textContent || ""; if (name) window.location.assign(sportsUrl(name)); });
        });
        document.querySelectorAll<HTMLElement>(".athlete-detail .page-head").forEach(head => {
          if (head.querySelector("[data-open-sports='true']")) return; const name = head.querySelector("h1")?.textContent?.trim(); if (!name) return;
          const button = document.createElement("button"); button.type = "button"; button.dataset.openSports = "true"; button.className = "outline"; button.textContent = "Ficha deportiva completa"; button.addEventListener("click", () => window.location.assign(sportsUrl(name))); head.appendChild(button);
        });
      }
      ...
      if (role === "owner" || role === "admin") {
        document.querySelectorAll<HTMLElement>(".athlete-detail .page-head").forEach(head => {
          if (head.querySelector("[data-open-sports='true']")) return; const name = head.querySelector("h1")?.textContent?.trim(); if (!name) return;
          const button = document.createElement("button"); button.type = "button"; button.dataset.openSports = "true"; button.className = "outline"; button.textContent = "Ver ficha deportiva"; button.addEventListener("click", () => window.location.assign(sportsUrl(name))); head.appendChild(button);
        });
        document.querySelectorAll<HTMLElement>(".group-roster span").forEach(row => {
          if (row.dataset.sportsLinked === "true") return; row.dataset.sportsLinked = "true"; row.style.cursor = "pointer"; row.setAttribute("role", "button"); row.setAttribute("tabindex", "0");
